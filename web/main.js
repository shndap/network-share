...
let peers = []
let nameCache = {}
const HARDCODED = [ 'Sahand','Amirhossein','M.Lashkari','Matin Bzr','Matin M.','Aref','Sina','Pouria','Parsa','Mgh' ]
const ROOM_W = 725
const ROOM_H = 513
const DESK_W = 150
const DESK_H = 150
...
function connect() {
	...
	ws.addEventListener('open', () => {
		let savedClientId = ''
		try { savedClientId = localStorage.getItem('clientId') || '' } catch {}
...
ws.addEventListener('message', (ev) => {
    ...
    try { if (msg && msg.type) console.log('ws message:', msg.type, msg) } catch {}
		switch (msg.type) {
			case 'welcome': {
        ...
        case 'peers': peers = normalizePeers(msg.peers); renderPeers(); break
        case 'layout': applyLayout(msg.positions || {}); break
        case 'layout_update': applyLayoutUpdate(msg.name, msg.x, msg.y); break
			case 'peer_left': peers = peers.filter(p => p.clientId !== msg.clientId); renderPeers(); break
        case 'relay': onRelay(msg.payload, msg.from, msg.fromName || ''); break
		}
	})
}
...
function onRelay(packet, from, fromName) {
    ...
    try { console.log('relay payload:', packet, 'from:', from, 'fromName:', fromName) } catch {}
	if (packet.phase === 'offer') {
		...
		li.innerHTML = `<div>
            ...
            <div class="meta">From: ${escapeHtml(senderName)} — ${readableSize(size)} — ${mime || 'application/octet-stream'}</div>
			<div class="progress"><div class="bar"></div></div>
		</div>
		<div class="actions"><button class="accept">Accept</button></div>`
		const bar = li.querySelector('.bar')
		const accept = () => {
			ws.send(JSON.stringify({ type: 'relay', to: from, payload: { kind: 'file', phase: 'accept', transferId }}))
		...
		let li = incoming.querySelector(`li[data-transfer-id="${packet.transferId}"]`)
		if (!li) {
			...
			li.innerHTML = `<div>
				...
				<div class="meta">${readableSize(packet.size || 0)} — ${packet.mime || 'application/octet-stream'}</div>
				<div class="progress"><div class="bar"></div></div>
			</div>
		...
		let state = li._state
		if (!state) {
			state = li._state = { chunks: [], received: 0, name: packet.name, size: packet.size, mime: packet.mime }
		}
		...
		const li = incoming.querySelector(`li[data-transfer-id="${packet.transferId}"]`)
		...
		downloadLink.href = url; downloadLink.download = name; downloadLink.className = 'download'
		...
		const cleanup = () => {
			...
			if (li._state) { li._state.chunks = []; li._state.received = size }
			li.remove()
		}
		openBtn.onclick = () => {
		...
		setTimeout(() => { try { URL.revokeObjectURL(url) } catch {} }, 5 * 60 * 1000)
	}
}
...
async function sendFile(file, bar, to) {
	...
	try {
		...
		if (to) {
			...
			toName = (target && target.name && target.name.trim()) ? target.name.trim() : shortId(to)
		}
		console.log(`sending ${file.name} from ${fromName} to ${toName}`)
	} catch {}
	const transferId = Math.random().toString(36).slice(2)
	ws.send(JSON.stringify({ type: 'relay', to: to || null, payload: { kind: 'file', phase: 'offer', transferId, name: file.name, size: file.size, mime: file.type || 'application/octet-stream' } }))
	...
	for (;;) {
		...
		if (done) break
		ws.send(JSON.stringify({ type: 'relay', to: to || null, payload: { kind: 'file', phase: 'chunk', transferId, data: Array.from(value), size: file.size, name: file.name, mime: file.type || 'application/octet-stream' } }))
		sent += value.length
		if (bar) bar.style.width = `${Math.round((sent / file.size) * 100)}%`
	}
	ws.send(JSON.stringify({ type: 'relay', to: to || null, payload: { kind: 'file', phase: 'complete', transferId } }))
}
...
function waitForAccept(transferId, to) {
	return new Promise((resolve) => {
		const onMsg = (ev) => {
			try {
				const msg = JSON.parse(ev.data)
				if (msg.type === 'relay' && (!to || msg.from === to) && msg.payload && msg.payload.kind === 'file' && msg.payload.phase === 'accept' && msg.payload.transferId === transferId) {
		...
		setTimeout(() => { ws.removeEventListener('message', onMsg); resolve(false) }, 30000)
	})
}

function renderPeers() { if (roomMap) renderRoomMap() }

function renderRoomMap() {
    roomMap.innerHTML = '<div class="drop-hint">Drop files here to send to all</div>'
    ...
    for (const name of HARDCODED) {
        const peer = peers.find(p => (p.name || '').trim() === name && !usedClientIds.has(p.clientId))
		const chip = document.createElement('div')
		const online = !!peer
		chip.className = 'desk' + ((peer && peer.clientId === clientId) ? ' self' : '') + (online ? ' online' : '')
		chip.dataset.name = name
        chip.dataset.clientId = online ? (usedClientIds.add(peer.clientId), peer.clientId) : ''
		chip.style.width = (DESK_W / ROOM_W * 100) + '%'
		chip.style.height = (DESK_H / ROOM_H * 100) + '%'
		const label = document.createElement('div')
		label.className = 'label'
		label.innerHTML = `${name}<span class="sub">${online ? shortId(peer.clientId) : 'offline'}</span>`
		...
		if (pos) {
			...
			chip.style.left = ((Math.random() * (ROOM_W - DESK_W)) / ROOM_W * 100) + '%'
			chip.style.top = ((Math.random() * (ROOM_H - DESK_H)) / ROOM_H * 100) + '%'
		}
		chip.ondragover = (e) => { e.preventDefault(); e.stopPropagation(); chip.classList.add('dragover') }
		chip.ondragleave = (e) => { e.stopPropagation(); chip.classList.remove('dragover') }
        chip.ondrop = (e) => {
			e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation?.(); chip.classList.remove('dragover')
			const files = e.dataTransfer?.files
			if (!files || files.length === 0) return
			const to = chip.dataset.clientId || null
			if (!to) return
            chip.classList.add('flash'); setTimeout(() => chip.classList.remove('flash'), 600)
    ...
    roomMap.ondragover = (e) => { e.preventDefault(); roomMap.classList.add('dragover'); roomMap.classList.add('broadcast') }
    roomMap.ondragleave = () => { roomMap.classList.remove('dragover'); roomMap.classList.remove('broadcast') }
    roomMap.ondrop = (e) => {
        e.preventDefault(); roomMap.classList.remove('dragover'); roomMap.classList.add('flash'); setTimeout(() => roomMap.classList.remove('flash'), 600)
        const files = e.dataTransfer?.files
        if (!files || files.length === 0) return
...
function savePositionByName(el) {
	...
	try { localStorage.setItem('positionsByName', JSON.stringify(positions)) } catch {}
	try {
		if (ws && ws.readyState === WebSocket.OPEN) {
			console.log('sending layout_update', el.dataset.name, x, y)
			ws.send(JSON.stringify({ type: 'layout_update', name: el.dataset.name, x, y }))
		}
	} catch {}
}

function loadPositionsByName() {
	try { return JSON.parse(localStorage.getItem('positionsByName') || '{}') } catch { return {} }
}
...
function applyLayoutUpdate(name, x, y) {
    ...
    const desk = roomMap && roomMap.querySelector(`.desk[data-name="${CSS.escape(name)}"]`)
    if (desk) {
    ...
    for (const g of guestPeers) {
        ...
        while (tries-- > 0) {
            ...
            const y = Math.random() * (ROOM_H - DESK_H)
            const rect = { x, y, w: DESK_W, h: DESK_H }
            const overlaps = usedRects.some(r => r && Math.abs((r.x + DESK_W/2) - (x + DESK_W/2)) < DESK_W && Math.abs((r.y + DESK_H/2) - (y + DESK_H/2)) < DESK_H)
            if (!overlaps) {
                circle.style.left = (x / ROOM_W * 100) + '%'
                circle.style.top = (y / ROOM_H * 100) + '%'
                usedRects.push({ x, y })
...
async function sendFiles(files, toClientId) { for (const f of files) await sendFile(f, null, toClientId) }
...
function normalizePeers(list) {
	...
	const normalized = list
		.map((p) => {
			if (!p) return null
			if (typeof p === 'string') return { clientId: p, name: '', mac: '', ip: '' }
			return {
				clientId: p.clientId || '',
...
function dedupeByClientId(list) {
	...
	for (const p of list) {
		...
		if (!existing) { map.set(p.clientId, p); continue }
		const score = (x) => (x.name && x.name.trim() ? 2 : 0) + (x.host && x.host.trim() ? 1 : 0)
...
function displayName(p){
	...
	const host = (p && typeof p.host === 'string' && p.host.trim()) ? p.host.trim() : ''
...
function rememberName(id, name) {
	...
	try { localStorage.setItem('nameCache', JSON.stringify(nameCache)) } catch {}
}
...
if (saveIdentBtn) saveIdentBtn.addEventListener('click', () => {
    identity = { name: nameInput?.value?.trim() || '', mac: '' }
	try { localStorage.setItem('identity', JSON.stringify(identity)) } catch {}
	if (ws && ws.readyState === WebSocket.OPEN) {
...
function upsertSelfInPeers() {
	...
	peers = (peers || []).map(p => {
		if (p && p.clientId === clientId) {
			found = true
			return { ...p, name: identity.name || p.name, mac: identity.mac || p.mac }
		}
		return p
	})
	if (!found) peers.push({ clientId, name: identity.name || '', mac: identity.mac || '', ip: '', host: '' })
	renderPeers()
}
