...
function ensureLayout(roomId) {
  if (!layouts.has(roomId)) {
    const base =
      roomId === "global" ? JSON.parse(JSON.stringify(DEFAULT_LAYOUT)) : {};
  ...
  ws.on("message", (message) => {
    ...
    switch (packet.type) {
      case "hello": {
        roomId = "global";
        clientId = String(
          packet.clientId || Math.random().toString(36).slice(2)
        );
        const identity = {
          clientId,
        ...
        try {
          ws.send(
            JSON.stringify({ type: "layout", positions: ensureLayout(roomId) })
          );
        } catch {}
        const peers = [...room]
          .map((s) => normalizeIdentity(s._identity))
          .filter(Boolean);
        broadcast(roomId, JSON.stringify({ type: "peers", peers }), null);
      ...
      case "identify": {
        if (!roomId) return;
        const name =
          typeof packet.identity?.name === "string" ? packet.identity.name : "";
        const mac =
          typeof packet.identity?.mac === "string" ? packet.identity.mac : "";
        ws._identity = {
          clientId,
          name,
          mac,
          ip: ws?._socket?.remoteAddress || ws._identity?.ip || "",
          host: ws._identity?.host || "",
        };
        broadcast(
          roomId,
          JSON.stringify({
            type: "peers",
            peers: [...ensureRoom(roomId)]
              .map((s) => normalizeIdentity(s._identity))
              .filter(Boolean),
          }),
          null
      ...
      case "relay": {
        ...
        const envelope = {
          type: "relay",
          from: clientId,
          fromName: (ws._identity && ws._identity.name) || "",
          to,
